...
struct node {
	...
	node(node const& rhs) : value(rhs.value), left(rhs.left), right(rhs.right), parent(rhs.parent) {}
};
...
//++ --
	...
	template<typename U>
	bool operator == (set_iterator<U> const& rhs) {
...
//++ --
	...
	template<typename U>
	bool operator == (const_set_iterator<U> const& rhs) {
		return data == rhs.data;
	}
...
public:
	...
	tree_set() : tree_size(0), fake_min(new node<T> ()) , fake_max(new node<T> ()) {
		fake_min->left = fake_min->parent = nullptr;
		fake_min->right = fake_max;

		fake_max->left = fake_max->right = nullptr;
		fake_max->parent = fake_min;
	}

	tree_set(tree_set<T> const& rhs) : tree_size(0), fake_min(new node<T> ()) , fake_max(new node<T> ()) {
		fake_min->left = fake_min->parent = nullptr;
		fake_min->right = fake_max;
	...
	tree_set(InputIterator first, InputIterator last) : tree_size(0), fake_min(new node<T> ()) , fake_max(new node<T> ()) {
		fake_min->left = fake_min->parent = nullptr;
		fake_min->right = fake_max;
	...
	void erase ( iterator pos ) {
		if (pos.data == fake_min || pos.data == fake_max) {
